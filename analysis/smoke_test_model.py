"""Smoke test: HierarchicalGNN forward with fixed-topology PE + node embeddings,
using the real frozen graph from graph.pt. Builds a 2-graph batch of synthetic
features and checks output shapes + that sign-flip augmentation runs in train mode.
...
K = NX * NY * NZ
E = ei1.shape[1]
node_in_dim, pe_dim = 11, pe1.shape[1]
...
for mode in ("train", "eval"):
    ...
    gp, npd = model(x, edge_index, edge_attr, batch,
                    ...
                    tile_x=tile_x, num_graphs=B, pe=pe, node_id=node_id)
...
gp, npd = model(x, edge_index, edge_attr, batch, tile_idx=tile_idx, tile_NX=tNX,
                tile_NY=tNY, tile_NZ=tNZ, tile_x=tile_x, num_graphs=B, pe=pe, node_id=node_id)
npd.sum().backward()
