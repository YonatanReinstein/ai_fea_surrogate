"""End-to-end batching smoke test: build Data objects with shared pe/node_id like
...
pe = g["pe"].float(); pe = (pe - pe.mean(0)) / (pe.std(0) + 1e-8)
...
x, edge_index, edge_attr, b, ti, nx, ny, nz, tx, pe_b, nid_b = gnn_input_fn(batch)

print(f"batch: x={tuple(x.shape)} pe={tuple(pe_b.shape)} node_id={tuple(nid_b.shape)}")
...
model = HierarchicalGNN(node_in_dim=node_in_dim, edge_in_dim=1, hidden_dim=64,
                        num_layers=6, pe_dim=pe.shape[1], num_pos_nodes=N)
gp, npd = model(x, edge_index, edge_attr, b, tile_idx=ti, tile_NX=nx, tile_NY=ny,
                tile_NZ=nz, tile_x=tx, num_graphs=batch.num_graphs, pe=pe_b, node_id=nid_b)
