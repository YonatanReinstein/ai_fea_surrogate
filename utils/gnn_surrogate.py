...
class GNN(torch.nn.Module):
    def __init__(self, in_features: int, out_features_node: int, out_features_global: int, hidden: int =128):
        super().__init__()
        self.g1 = GraphConv(in_features, hidden)
        self.g2 = GraphConv(hidden, hidden)
        self.node_head = torch.nn.Linear(hidden, out_features_node)  # [ux, uy, uz, σ_vm]
